...
self.addEventListener("fetch", event => {
    ...
    const updateAssetName = TEXT_ASSETS.has(fileName) ? fileName : "";

    event.respondWith(staleWhileRevalidate(event, request, updateAssetName));
});

async function staleWhileRevalidate(event, request, updateAssetName) {
    const cache = await caches.open(CACHE_NAME);
    ...
    const refreshPromise = fetch(new Request(request, { cache: "no-store" }))
        .then(async response => {
            ...
            const changed = cached && updateAssetName
                ? await responsesDiffer(cached, response)
                : false;

            await cache.put(request, response.clone());
            if (changed) await broadcastUpdate(updateAssetName);
            return response;
        })
        .catch(error => {
