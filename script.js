...
function rebuildDataIndexes() {
    ...
    for (const [region, apartmentSet] of apartmentsByRegionSet.entries()) {
        indexes.apartmentsByRegion.set(region, sortApartmentsOfficeLast([...apartmentSet]));
    }
...
function renderRegionButtons() {
    ...
    elements.buttonGrid.appendChild(fragment);
    renderGpsButtons();
}
...
function renderApartmentButtons() {
    const apartments = sortApartmentsOfficeLast(
...
function sortApartmentsOfficeLast(values) {
    const normalApartments = new Set();
    const officeApartments = new Set();

    for (const value of values || []) {
        const apartment = normalizeApartmentValue(value);
        if (!apartment) continue;
        if (isOfficeApartmentMarker(apartment)) officeApartments.add(apartment);
        else normalApartments.add(apartment);
    }
...
function renderPasswordCards() {
    ...
    const fragment = document.createDocumentFragment();
    for (const record of records) fragment.appendChild(createPasswordCard(record));
...
function hasOpenModal() {
    return [elements.commonEditorModal, elements.addPwdModal, elements.deletePwdModal, elements.historyModal]
        .some(modal => modal && modal.style.display === "flex");
}

function scheduleSafeAppReload() {
    if (state.appReloadScheduled) return;
    state.appReloadScheduled = true;

    const tryReload = () => {
        if (state.syncProcessing || hasOpenModal()) {
            window.setTimeout(tryReload, 800);
            return;
        }

        const lastReload = Number(sessionStorage.getItem("gimpoB_asset_reload_time") || 0);
        if (Date.now() - lastReload < 10000) {
            state.appReloadScheduled = false;
            return;
        }

        sessionStorage.setItem("gimpoB_asset_reload_time", String(Date.now()));
        window.location.reload();
    };

    window.setTimeout(tryReload, 500);
}
...
if ("serviceWorker" in navigator) {
    navigator.serviceWorker.addEventListener("message", event => {
        ...
        const fileName = cleanText(data.pathname).split("/").pop();
        if (["index.html", "script.js", "style.css"].includes(fileName)) scheduleSafeAppReload();
    });
