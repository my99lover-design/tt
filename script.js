...
async function processPendingQueue() {
    ...
    try {
        await requestApi(operation.action, operation.payload);
...
function renderGpsButtons() {
    ...
    ).slice(0, APP_CONFIG.GPS_BUTTON_COUNT);
    ...
    for (const item of nearbyApartments) {
        ...
        button.textContent = `${item.apartment}\n${distanceText}`;
...
function getNearbyApartments(currentLatitude, currentLongitude) {
    const uniqueApartments = new Map();

    for (const record of state.records) {
        const exactName = normalizeApartmentName(record.apartment);
        if (!exactName || uniqueApartments.has(exactName)) continue;
        uniqueApartments.set(exactName, { region: record.region, apartment: record.apartment });
    }

    const results = [];

    for (const [exactName, apartmentInfo] of uniqueApartments) {
        const locationEntry = state.locationMap.get(exactName);
...
function initializeModalEvents() {
    const modals = [elements.commonEditorModal, elements.addPwdModal, elements.deletePwdModal];
...
function closeTopModal() {
    const modals = [elements.deletePwdModal, elements.addPwdModal, elements.commonEditorModal];
