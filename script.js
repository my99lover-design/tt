...
function renderGpsButtons() {
    ...
    for (const item of nearbyApartments) {
        ...
        const displayApartment = item.isOffice && !item.apartment.includes("오피") ? `오피·${item.apartment}` : item.apartment;
        button.textContent = `${displayApartment}\n${distanceText}`;
        button.title = `${item.region} · ${item.apartment} · ${distanceText}`;
...
function getNearbyApartments(currentLatitude, currentLongitude, buttonCount = APP_CONFIG.GPS_BUTTON_COUNT) {
    ...
    for (const record of state.records) {
        const exactName = normalizeApartmentName(record.apartment);
        const region = cleanText(record.region);
        if (!exactName || !region) continue;

        const uniqueKey = `${region.normalize("NFC")}\u0000${exactName}`;
        if (uniqueApartments.has(uniqueKey)) continue;

        uniqueApartments.set(uniqueKey, {
            region,
            apartment: record.apartment,
            exactName,
            isOffice: isOfficeRegion(region)
        });
    }

    const results = [];

    for (const apartmentInfo of uniqueApartments.values()) {
        const locationEntry = state.locationMap.get(apartmentInfo.exactName);
        ...
        results.push({
            ...
            isOffice: apartmentInfo.isOffice
        });
    }

    results.sort((a, b) => a.distance - b.distance || naturalCompare(a.apartment, b.apartment));
    ...
    return selected.sort((a, b) => a.distance - b.distance || naturalCompare(a.apartment, b.apartment)).slice(0, limit);
}
